...
function loadTable() {
    ...
    xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        for (let object of objects) {
          ...
          trHTML +=
            '<td><button type="button" class="btn btn-outline-info bg-dark" onclick="showUserEditBox(' +
            object["id"] +
            ')">Edit</button>';
          trHTML +=
            '<button type="button" class="btn btn-outline-danger bg-dark" onclick="userDelete(' +
            object["id"] +
            ')">Del</button></td>';
          trHTML += "</tr>";
        }
  ...
  //Inspect this region, username and email are not working properly
  //The username is registering in the email box and the api does not include usernames
  function userCreate() {
    ...
    xhttp.send(
      JSON.stringify({
        fname: fname,
        lname: lname,
        username: email, //changing the value of property 'username' to 'email' fixes the placement issue mentioned on line 54/55
        email: username,
        avatar: "https://th.bing.com/th/id/OIP.QXRHB_F0is6F15ZNw5SutwHaLc?pid=ImgDet&rs=1",
      })
    );
    xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
